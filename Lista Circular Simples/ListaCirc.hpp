...
template<typename T> /*!< template generico do tipo T*/
    // Classe para a implementação de uma Lista Circular
    /*!
...
class ListaCirc {
 private:
	Elemento<T>* sentinela; /*< Criando um ponteiro para ser a sentinela da lista*/
	int tamanho; /*< Variavel que indica qual o tamanho atual da lista */
	Elemento<T>* cabeca; /*< Criando um ponteiro para ser a cabeça da lista*/
 ...
 public:
	...
	int contador = 0;
	    if (listaVazia()) {
	        ...
	        Elemento<T> *auxiliar = sentinela->getProximo();
	...
	T mostra(int pos) {
        if (listaVazia()) {
            ...
            Elemento<T> *aux = sentinela->getProximo();
            for (int i = 0; i < pos; i++) {
                aux = aux->getProximo();
            }
            return (aux->getInfo());
